...
# --- (NOVO) Função Input Genérica ---
def get_text_input(surface, prompt):
    """Obtém texto do usuário (para Nome e IP)."""
    input_text = ''
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    # Permite IP vazio (usa localhost), mas exige nome
                    if input_text or "IP" in prompt:
                        active = False
                elif event.key == pygame.K_BACKSPACE:
                    input_text = input_text[:-1]
                else:
                    # Limita o tamanho para evitar problemas (opcional)
...
# --- Funções de Desenho ---
# (draw_bottle, draw_player, draw_base, draw_tooltip, draw_shop, draw_weapon_shop
#  são idênticas à versão anterior, omitidas para economizar espaço)
...
def draw_base(surface, player_data):
    ...
    for bottle_data in player_data["equipped_slots_data"]: draw_bottle(surface, bottle_data)
...
def draw_ui(surface, players_data_list, my_id):
    """(MUDANÇA) Mostra nome em vez de PX."""
    # Controles locais
    if players_data_list[my_id]:
        p_data = players_data_list[my_id]
        controls = small_font.render(CONTROLS_TEXT, True, p_data['color'])
        base_rect = pygame.Rect(p_data["base_rect_data"])
        pos_x = base_rect.left if my_id in [0, 2] else base_rect.right - controls.get_width()
        surface.blit(controls, (pos_x, base_rect.bottom + 5))

    # Texto do escudo
    def get_shield_text(p_data):
        text = ""
        if p_data["shield_active"]:
            text = f"Escudo: {p_data['shield_timer_frames'] // FPS}s"
        elif p_data["shield_cooldown_frames"] > 0:
            text = f"Escudo CD: {(p_data['shield_cooldown_frames'] // FPS) + 1}s"
        return small_font.render(text, True, SHIELD_BLUE if text else WHITE)

        # Infos dos jogadores

    if players_data_list[0]:
        p = players_data_list[0];
        name = p['name'][:10]  # Limita nome
        money = font.render(f"{name}: ${p['money']:.2f}", True, WHITE)  # Mostra nome
        rays = small_font.render(f"Raios: {p['consumables']['Raio Orbital']}", True, WHITE)
        shield = get_shield_text(p);
...
def redraw_window(surface, game_state, my_id):
    ...
    for b_data in bottles:
        ...
        surface.blit(price, price.get_rect(midbottom=b_rect.midtop, y=b_rect.top - 2))

    # Bases e Jogadores
    for p_data in players: draw_base(surface, p_data); draw_player(surface, p_data)
    draw_ui(surface, players, my_id)

    # Tooltip (fora da esteira)
    ...
    for p in players:
        if p:
            if p["carrying_bottle_data"]:
                ...
                all_bottles.append(temp)
            all_bottles.extend([b for b in p["equipped_slots_data"] if b])
    for b in all_bottles:
        if pygame.Rect(b["rect_data"]).collidepoint(mouse): hover = b; break
...
def main():
    ...
    # --- (NOVO) Obtém Nome e IP ---
    player_name = get_text_input(screen, "Digite seu Nome e pressione Enter:")
    if not player_name: player_name = "Jogador"  # Nome default

    server_ip = get_text_input(screen, "Digite o IP do Servidor (ou Enter para localhost):")
    # Usa localhost se IP for deixado em branco
    server_ip = server_ip if server_ip else "127.0.0.1"

    try:
        # --- (NOVO) Passa o nome para Network ---
        n = Network(server_ip, player_name, PORT)
        my_id = n.get_player_id()
        if my_id is None: raise ConnectionRefusedError("Servidor recusou a conexão (talvez cheio?)")
        print(f"Você é o Jogador {my_id + 1} ({player_name})")
    except Exception as e:
        print(f"Não foi possível conectar ao servidor {server_ip}:{PORT}")
        print(f"Erro: {e}")
        print("Verifique IP, Firewall e se server.py está rodando.")
        running = False;
    ...
    while running:
        ...
        # 3. Envia inputs
        if not n.send(inputs): print("Falha envio. Desconectando."); running = False; break
