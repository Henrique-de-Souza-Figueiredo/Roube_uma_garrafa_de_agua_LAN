...
SHIELD_BLUE = (100, 100, 255)  # Cor do escudo e UI
RED = (255, 0, 0)
GREEN = (0, 200, 0)
...
# --- (MUDANÇA) Loja de Upgrades (Visual) ---
...
def draw_base(surface, player_data):
    """(MUDANÇA) Desenha base, botão de escudo e efeito visual do escudo."""
    if not player_data: return
    ...
    cooldown_frames = player_data["shield_cooldown_frames"]
    button_color = GREEN if cooldown_frames <= 0 else RED  # Verde (Pronto) / Vermelho (Cooldown)
    pygame.draw.rect(surface, button_color, button_rect)
    pygame.draw.rect(surface, WHITE, button_rect, 1)

    # Efeito visual do escudo (se ativo)
    if player_data["shield_active"]:
        shield_surface = pygame.Surface((base_rect.width, base_rect.height), pygame.SRCALPHA)
        # Pisca um pouco baseado no timer para dar feedback
        alpha = 100 + (player_data['shield_timer_frames'] % 15) * 2  # Alfa entre 100 e 130
        shield_surface.fill((*SHIELD_BLUE, alpha))
        surface.blit(shield_surface, base_rect.topleft)
...
def draw_weapon_shop(surface):
    ...
    surface.blit(shop_title, title_rect)
    # --- (MUDANÇA) Itera sobre os dados atualizados da loja ---
...
def draw_ui(surface, players_data_list, my_id):
    """(MUDANÇA) Mostra os timers do escudo."""
    ...
    if players_data_list[my_id]:
        ...
        pos_x = base_rect.left if my_id in [0, 2] else base_rect.right - controls_text.get_width()
        pos_y = base_rect.bottom + 5
        surface.blit(controls_text, (pos_x, pos_y))

    # Função auxiliar para texto do escudo
    def get_shield_text(p_data):
        shield_text = ""
        if p_data["shield_active"]:
            shield_seconds = p_data['shield_timer_frames'] // FPS
            shield_text = f"Escudo: {shield_seconds}s"
        elif p_data["shield_cooldown_frames"] > 0:
            # Mostra segundos restantes do cooldown
            cooldown_seconds = (p_data['shield_cooldown_frames'] // FPS) + 1
            shield_text = f"Escudo CD: {cooldown_seconds}s"
        # Renderiza com a cor SHIELD_BLUE
        return small_font.render(shield_text, True, SHIELD_BLUE if shield_text else WHITE)

        # Desenha as informações dos jogadores

    if players_data_list[0]:
        p_data = players_data_list[0]
        money = font.render(f"P1: ${p_data['money']:.2f}", True, WHITE)
        rays = small_font.render(f"Raios: {p_data['consumables']['Raio Orbital']}", True, WHITE)
        shield = get_shield_text(p_data)
        surface.blit(money, (10, 10));
        surface.blit(rays, (10, 30));
        surface.blit(shield, (10, 50))

    if players_data_list[1]:
        p_data = players_data_list[1]
        money = font.render(f"P2: ${p_data['money']:.2f}", True, WHITE)
        rays = small_font.render(f"Raios: {p_data['consumables']['Raio Orbital']}", True, WHITE)
        shield = get_shield_text(p_data)
        surface.blit(money, (SCREEN_WIDTH - money.get_width() - 10, 10))
        surface.blit(rays, (SCREEN_WIDTH - rays.get_width() - 10, 30))
        surface.blit(shield, (SCREEN_WIDTH - shield.get_width() - 10, 50))

    if players_data_list[2]:
        p_data = players_data_list[2]
        base_rect = pygame.Rect(p_data["base_rect_data"])
        money = font.render(f"P3: ${p_data['money']:.2f}", True, WHITE)
        rays = small_font.render(f"Raios: {p_data['consumables']['Raio Orbital']}", True, WHITE)
        shield = get_shield_text(p_data)
        surface.blit(money, (10, base_rect.top - 60))
        surface.blit(rays, (10, base_rect.top - 40))
        surface.blit(shield, (10, base_rect.top - 20))

    if players_data_list[3]:
        p_data = players_data_list[3]
        base_rect = pygame.Rect(p_data["base_rect_data"])
        money = font.render(f"P4: ${p_data['money']:.2f}", True, WHITE)
        rays = small_font.render(f"Raios: {p_data['consumables']['Raio Orbital']}", True, WHITE)
        shield = get_shield_text(p_data)
        surface.blit(money, (SCREEN_WIDTH - money.get_width() - 10, base_rect.top - 60))
        surface.blit(rays, (SCREEN_WIDTH - rays.get_width() - 10, base_rect.top - 40))
        surface.blit(shield, (SCREEN_WIDTH - shield.get_width() - 10, base_rect.top - 20))
...
def redraw_window(surface, game_state, my_id):
    ...
    for player_data in players_data_list:
        draw_base(surface, player_data)  # Agora desenha escudo
        draw_player(surface, player_data)

    draw_ui(surface, players_data_list, my_id)  # Agora desenha timers
    ...
    for p_data in players_data_list:
        if p_data:
            ...
            for b_data in p_data["equipped_slots_data"]:
                if b_data: all_bottles_to_check.append(b_data)
    ...
    if hover_bottle_data: draw_tooltip(surface, hover_bottle_data, mouse_pos)
...
def main():
    ...
    try:
        ...
        print(f"Não foi possível conectar: {e}\nVerifique IP e se server.py está rodando.");
        running = False;
    ...
    while running:
        ...
        if new_game_state:
            ...
            print("Falha inicial."); running = False; break
        elif new_game_state is None:
            print("Servidor desconectado."); running = False; break

        # 2. Coleta inputs
        interact_pressed, use_item_pressed = False, False
        for event in pygame.event.get():
            if event.type == pygame.QUIT: running = False
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_f: interact_pressed = True
                if event.key == pygame.K_g: use_item_pressed = True
        keys_pressed = pygame.key.get_pressed()
        input_data = {'keys': keys_pressed, 'interact': interact_pressed, 'use_item': use_item_pressed}

        # 3. Envia inputs
        if not n.send(input_data): print("Falha ao enviar. Desconectando."); running = False; break
